...
def reproject(
    ...
    :param cam_distortion_k (T,2)
    :param cam_distortion_p (T,2)
    """

    B, T, N, _ = points3d.shape

    assert cam_R.shape == (B, T, 3, 3), f"error with cam_R shape: {cam_R.shape}"
    ...
    # Apply distortion
    if cam_distortion_k is not None:
        points2d = apply_distortion(points2d, cam_distortion_k, cam_distortion_p)

    # Apply camera intrinsics
    points2d_final = (
        cam_f[None, :, None] * points2d + cam_center[None, :, None]
    )  # Scale and offset

    return points2d_final


def apply_distortion(points2d, k, p=None):
    """
    Apply radial and tangential distortion correction.
    :param points2d: Normalized image points (B, T, N, 2)
    :param k: Radial distortion coefficients (T,3)
    :param p: optional Tangential distortion coefficients (T,2)
    """
    ...
    r6 = r2 * r4
    # Radial distortion
    radial = 1 + k[0, 0] * r2 + k[0, 1] * r4 + k[0, 2] * r6
    x_distorted = x * radial
    y_distorted = y * radial

    # Tangential distortion
    if p is not None:
        x_distorted += 2 * p[0, 0] * x * y + p[0, 1] * (r2 + 2 * x**2)
        y_distorted += p[0, 0] * (r2 + 2 * y**2) + 2 * p[0, 1] * x * y

    return torch.stack([x_distorted, y_distorted], dim=-1)


# def reproject(points3d, cam_R, cam_t, cam_f, cam_center, distortion=None):
#     """
#     Reproject points3d into the scene cameras with radial distortion.
#     :param points3d (B, T, N, 3): 3D points to be projected
#     :param cam_R (B, T, 3, 3): Camera rotation matrices
#     :param cam_t (B, T, 3): Camera translation vectors
#     :param cam_f (T, 2): Focal lengths for each camera
#     :param cam_center (T, 2): Principal points (camera center) for each camera
#     :param distortion: (T,3): Radial distortion coefficients k1, k2, k3
#     """
#     B, T, N, _ = points3d.shape
#     points3d = torch.einsum("btij,btnj->btni", cam_R, points3d)
...
#         # Compute radius squared (r^2) for radial distortion
#         r2 = torch.sum(points2d ** 2, axis=-1)
#         r4 = r2 ** 2
#         r6 = r2 ** 3

#         radial_distortion = 1 + distortion['k1'] * r2 + distortion['k2'] * r4 + distortion['k3'] * r6
#         points2d = points2d * radial_distortion[..., None]

#     # Apply camera intrinsics (focal length and camera center)
