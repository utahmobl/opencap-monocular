...
class OptimizePose:
    ...
    def optimize(self):
        ...
        # Optimization loop
        objective_values = torch.zeros(self.iterations, device=self.device)
        ...
        for i in range(self.iterations):
            ...
            if nan_recovery_count[0] >= max_nan_recoveries:
                ...
                # Fill remaining objective values with last loss
                for j in range(i, self.iterations):
                    objective_values[j] = (
                        self.loss.clone().detach().cpu()
